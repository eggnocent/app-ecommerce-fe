...
import { ORDER_STATUS_CANCELED, ORDER_STATUS_DONE, ORDER_STATUS_PAID, ORDER_STATUS_SHIPPED, ORDER_STATUS_UNPAID } from "../../const/order";
...
function AdminOrderDetail() {
    ...
    const updateStatusHandler = async () => {
        ...
        await Swal.fire({
            icon: 'success',
            text: 'Status telah sukses diubah'
        });
                ...
                <div className="bg-white p-4 rounded-3">
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-6 mb-4">
                            ...
                            <div className="p-3 border rounded">
                                <p className="mb-2"><strong>Status Saat Ini:</strong><OrderStatusBadge code={orderStatusCode}/> </p>
                                ...
                                {[ORDER_STATUS_UNPAID, ORDER_STATUS_PAID, ORDER_STATUS_SHIPPED].includes(orderStatusCode) && (
                                    ...
                                    >
                                        ...
                                        {orderStatusCode === ORDER_STATUS_PAID && <option value={ORDER_STATUS_SHIPPED}>Dikirim</option>}
                                        {orderStatusCode === ORDER_STATUS_SHIPPED && <option value={ORDER_STATUS_DONE}>Selesai</option>}
                                    </select>
                                    <button className="btn btn-primary w-100" 
                                    onClick={updateStatusHandler} 
                                    disabled={newOrderStatus === '' || updateStatusApi.isLoading}>Perbarui Status</button>
                                </div>
                                )}
                            </div>
                        </div>
